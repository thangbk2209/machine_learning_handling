...
# from model.fuzzy_encoder_decoder_uber import Model as FED
from model.fuzzy_inference import Model as FuzzyInference
import traceback
...
def train_model(item):
    ...
    model = FuzzyInference(dataset_original, prediction_data, external_feature, train_size, valid_size, 
    ...
    # file_name = str(sliding_encoder) + '-' + str(sliding_decoder) + '-' + str(batch_size) + '-' + name_LSTM + '-' + str(activation)+ '-' + str(optimizer) + '-' + str(input_dim) +'-'+str(number_out_decoder) +'-'+str(dropout_rate)
