...
class Model:
    ...
    def fit(self):
        ...
        x3 = tf.placeholder("float",[None, int(self.sliding_inference/self.input_dim), self.input_dim])
        ...
        # state = tf.shape(x3)[0]
        state = tf.reshape(state_encoder[-1].h, [tf.shape(x3)[0],self.sliding_inference,self.input_dim])
        input_inference = tf.concat([x3,state],1)
        input_inference = tf.reshape(input_inference,[tf.shape(x3)[0], 2 * self.sliding_inference * self.input_dim])
        ...
        hidden_value = tf.layers.dense(input_inference, 20, activation=tf.nn.sigmoid)
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(self.epochs_encoder_decoder):
                ...
                if epoch % self.display_step == 0:
                    ...
                    print (sess.run(outputs_decoder,feed_dict={x1: batch_xs_encoder,x2: batch_xs_decoder}))
                    print (val_cost)
                    cost_valid_set.append(val_cost)
                    print ("Training phase encoder-decoder finished")
